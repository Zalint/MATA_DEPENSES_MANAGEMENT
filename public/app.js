...
// Initialize Audit Flux Module
function initAuditFluxModule() {
    console.log('🔍 CLIENT: Initialisation du module Audit Flux');
    
    // Vérifier les permissions d'accès
    const currentUser = getCurrentUser();
    if (!currentUser || !['admin', 'directeur_general', 'pca', 'directeur'].includes(currentUser.role)) {
        console.log('❌ CLIENT: Accès Audit Flux refusé pour l\'utilisateur:', currentUser);
        return;
    }
    
    console.log('✅ CLIENT: Accès Audit Flux autorisé pour:', currentUser.username);
    
    // Charger la liste des comptes pour le sélecteur
    loadAccountsForAudit();
    
    // Attacher les event listeners
    setupAuditFluxEventListeners();
}

// Charger les comptes disponibles pour l'audit
async function loadAccountsForAudit() {
    console.log('📋 CLIENT: Chargement des comptes pour audit');
    ...
    try {
        ...
        console.log(`📋 CLIENT: ${accounts.length} comptes chargés pour audit`);
        
        const accountSelect = document.getElementById('audit-account-select');
        if (!accountSelect) {
            console.error('❌ CLIENT: Élément audit-account-select non trouvé');
            return;
        }
        ...
        accounts.forEach(account => {
            ...
            option.textContent = `${account.account_name} (${account.account_type}) - ${formatCurrency(account.current_balance)}`;
            accountSelect.appendChild(option);
        });
        
        console.log('✅ CLIENT: Comptes ajoutés au sélecteur d\'audit');
        
    } catch (error) {
        console.error('❌ CLIENT: Erreur chargement comptes pour audit:', error);
        showNotification('Erreur lors du chargement des comptes', 'error');
    }
}

// Configurer les event listeners pour Audit Flux
function setupAuditFluxEventListeners() {
    console.log('🔗 CLIENT: Configuration des event listeners Audit Flux');
    
    // Bouton d'exécution d'audit
    const executeAuditBtn = document.getElementById('execute-audit-btn');
    if (executeAuditBtn) {
        executeAuditBtn.addEventListener('click', executeAccountAudit);
        console.log('✅ CLIENT: Event listener audit ajouté');
    }
    ...
    if (exportCsvBtn) {
        ...
        console.log('✅ CLIENT: Event listener export CSV ajouté');
    }
    
    // Bouton d'affichage SQL
    const showSqlBtn = document.getElementById('show-sql-query');
    if (showSqlBtn) {
        showSqlBtn.addEventListener('click', showSQLQuery);
        console.log('✅ CLIENT: Event listener SQL query ajouté');
    }
    
    // Bouton de fermeture du modal SQL
    const closeSqlModalBtn = document.getElementById('close-sql-modal');
    if (closeSqlModalBtn) {
        closeSqlModalBtn.addEventListener('click', closeSQLModal);
        console.log('✅ CLIENT: Event listener fermeture modal SQL ajouté');
    }
    
    // Bouton de copie de la requête SQL
    const copySqlBtn = document.getElementById('copy-sql-btn');
    if (copySqlBtn) {
        copySqlBtn.addEventListener('click', copySQLQuery);
        console.log('✅ CLIENT: Event listener copie SQL ajouté');
    }
    
    console.log('✅ CLIENT: Tous les event listeners Audit Flux configurés');
}

// Variables globales pour l'audit
let currentAuditData = null;

// Exécuter l'audit d'un compte
async function executeAccountAudit() {
    console.log('🔍 CLIENT: Exécution de l\'audit de compte');
    
    const accountSelect = document.getElementById('audit-account-select');
    const startDateInput = document.getElementById('audit-start-date');
    const endDateInput = document.getElementById('audit-end-date');
    const executeBtn = document.getElementById('execute-audit-btn');
    
    if (!accountSelect || !startDateInput || !endDateInput) {
        console.error('❌ CLIENT: Éléments d\'audit manquants');
        return;
    }
    
    const accountId = accountSelect.value;
    if (!accountId) {
        showNotification('Veuillez sélectionner un compte à auditer', 'error');
        return;
    }
    ...
    executeBtn.textContent = 'Audit en cours...';
    ...
    try {
        ...
        console.log(`🔍 CLIENT: Requête audit vers: ${url}`);
        ...
        console.log('✅ CLIENT: Données d\'audit reçues:', auditData);
        ...
        console.error('❌ CLIENT: Erreur lors de l\'audit:', error);
// État global de l'application
let currentUser = null;
let categories = [];
let users = [];
